...
export function Sidebar({ activeId, onSelect, isOpen, onClose }: Props) {
  ...
  return (
    <>
      {/* Backdrop — mobile only, only shown while the drawer is open */}
      {isOpen && (
      ...
      <nav
        ...
        className={`fixed inset-y-0 left-0 z-40 flex h-full w-72 max-w-[85vw] shrink-0 transform flex-col border-r border-[var(--ink-700)] bg-[var(--ink-900)] transition-transform duration-200 ease-out lg:static lg:z-auto lg:w-64 lg:max-w-none lg:translate-x-0 ${
